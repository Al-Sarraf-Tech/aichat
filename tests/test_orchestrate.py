...
class TestOrchestrateSchema:
    ...
    def _get_schema(self):
        # orchestrate is now the 'planner' mega-tool
        return next((t for t in _TOOLS if t["name"] == "planner"), None)

    def test_orchestrate_in_tools_list(self):
        # orchestrate is now an action of the 'planner' mega-tool
        names = {t["name"] for t in _TOOLS}
        assert "planner" in names, f"planner mega-tool not in _TOOLS (found: {sorted(names)})"
        schema = self._get_schema()
        assert schema is not None
        actions = schema["inputSchema"]["properties"]["action"]["enum"]
        assert "orchestrate" in actions, f"orchestrate action missing from planner: {actions}"

    def test_steps_param_required(self):
        schema = self._get_schema()
        assert schema is not None
        # steps is a property of the planner tool (used for orchestrate action)
        props = schema["inputSchema"]["properties"]
        assert "steps" in props, f"steps not in planner schema properties: {list(props.keys())}"
    ...
    def test_total_tools_count(self):
        """Platform now has exactly 21 tools (16 mega + 4 team)."""
        assert len(_TOOLS) == 21, f"Expected 21 tools (16 mega + 4 team + 1 workspace), got {len(_TOOLS)}"
...
class TestOrchestrateSmoke:
    ...
    def test_orchestrate_appears_in_tool_list(self):
        ...
        # orchestrate is an action of the planner mega-tool
        assert "planner" in names, f"planner missing from live tools/list: {sorted(names)}"
        planner = next(t for t in tools if t["name"] == "planner")
        actions = planner["inputSchema"]["properties"]["action"]["enum"]
        assert "orchestrate" in actions, f"orchestrate action missing from planner: {actions}"
