...
_TOOLS: list[dict[str, Any]] = [
        ...
        "inputSchema": {
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": (
                        "Image filename or path to analyze "
                        "(supports bare filename, /workspace/..., or host path)."
                    ),
                },
                "reference_path": {
                    "type": "string",
                    "description": (
                        "Optional second image containing a reference face. "
                        "When provided, each detected face in 'path' is scored against it."
                    ),
                },
                "style": {
                    "type": "string",
                ...
                "match_threshold": {
                    "type": "number",
                    "description": (
                        "Similarity threshold (0.0-1.0) for same-person match, default 0.82."
                    ),
                },
                "min_face_size": {
                    "type": "integer",
                    "description": "Minimum face size in pixels for detection (default 40; anime preset uses 20).",
                },
                "min_neighbors": {
                    "type": "integer",
                    "description": (
                        "How many neighbor detections a candidate must have to be kept (default 5). "
                        "Lower values (1-2) detect more faces but may add false positives. "
                        "The anime preset automatically sets this to 2."
                    ),
                },
                "scale_factor": {
                    "type": "number",
                    "description": (
                        "Image scale step between detection passes (default 1.1). "
                        "Smaller values (e.g. 1.05) are slower but catch more sizes. "
                        "The anime preset automatically sets this to 1.05."
                    ),
                },
                "annotate": {
                    "type": "boolean",
                    "description": "If true (default), draw face boxes/labels on the output image.",
                },
            },
            "required": ["path"],
        },
    ...
    },
    {
        ...
        "description": (
            ...
            "Returns stdout, stderr, exit code, and duration. "
            "Has access to the full Node.js standard library (fs, path, crypto, http, etc.). "
            "The working directory is /workspace. Files from face_recognize and other tools "
            "are accessible at /workspace/<filename>. "
            "Use for file operations, data processing, JSON manipulation, or any task "
            "where JavaScript is more natural than Python."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string",
                    "description": "JavaScript (CommonJS) source code to execute with Node.js.",
                },
                "timeout": {
                    "type": "integer",
                    "description": "Wall-clock timeout in seconds (default 30, max 120).",
    ...
    # ── Reasoning / Research ────────────────────────────────────────────────
    {
        "name": "think",
        "description": (
            "Scratchpad for explicit step-by-step reasoning — call this BEFORE any "
            "irreversible tool (file writes, API calls, destructive operations). "
            "Records a thought/hypothesis/plan in the tool trace without any side effects. "
            "Returns the thought exactly as given. Works with any model that supports tools."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "thought": {
                    "type": "string",
                    "description": "Reasoning, hypothesis, step-by-step plan, or intermediate conclusion.",
                },
            },
            "required": ["thought"],
        },
    },
    {
        "name": "deep_research",
        "description": (
            "Multi-hop autonomous research: iteratively searches the web, fetches and extracts "
            "full article text from the top sources, identifies follow-up angles, and returns "
            "a structured report with citations. Far more thorough than web_search alone. "
            "Use for complex questions, technology overviews, competitive analysis, or any topic "
            "requiring synthesis across multiple sources. Runs 2–3 search rounds automatically."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "depth": {
                    "type": "integer",
                    "description": "Search rounds (1–3, default 2). Higher = slower but more thorough.",
                },
                "max_sources": {
                    "type": "integer",
                    "description": "Max sources to fetch per round (default 4).",
                },
            },
            "required": ["question"],
        },
    ...
    # ── Live Data ────────────────────────────────────────────────────────────
    {
        "name": "realtime",
        "description": (
            "Live real-world data — zero API keys required. "
            "Types: "
            "'time' — current time+date in any timezone (e.g. 'America/New_York'); "
            "'weather' — current conditions + 7-day forecast for any city; "
            "'stock' — price, change, volume for any ticker (AAPL, TSLA, BTC-USD); "
            "'crypto' — USD/EUR price for any coin on CoinGecko (bitcoin, ethereum, solana); "
            "'forex' — live exchange rate between any two currencies (USD/EUR, GBP/JPY)."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "type": {
                    "type": "string",
                    "enum": ["time", "weather", "stock", "crypto", "forex"],
                    "description": "Data type to fetch.",
                },
                "query": {
                    "type": "string",
                    "description": (
                        "time → IANA timezone (e.g. 'America/New_York', 'Asia/Tokyo'); "
                        "weather → city name (e.g. 'London', 'Tokyo'); "
                        "stock → ticker (e.g. 'AAPL', 'MSFT', 'BTC-USD'); "
                        "crypto → coin id (e.g. 'bitcoin', 'ethereum', 'solana'); "
                        "forex → 'USD/EUR' or 'GBP/JPY' format."
                    ),
                },
            },
            "required": ["type", "query"],
    ...
    },
    {
        "name": "news_search",
        "description": (
            "Search and aggregate current news from major RSS feeds: BBC, The Guardian, "
            "Reuters, AP, Hacker News, Ars Technica, TechCrunch. No API key required. "
            "Filter by keyword or leave empty for top headlines. "
            "Use for breaking news, tech trends, current events, or monitoring a topic."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Keyword filter (empty = top headlines from all sources).",
                },
                "sources": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Specific sources: bbc, guardian, hackernews, arstechnica, techcrunch (default: all).",
                },
                "limit": {
                    "type": "integer",
    ...
    },
    {
        "name": "wikipedia",
        "description": (
            "Search Wikipedia and return article summaries or full text. "
            "More reliable than web search for established facts, biographies, history, science. "
            "Returns intro summary by default; set full_article=true for complete text."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Search term or exact article title.",
                },
                "lang": {
                    "type": "string",
                ...
                "full_article": {
                    "type": "boolean",
                    "description": "Return full article text (default false = intro summary only).",
                },
            },
            "required": ["query"],
        },
    },
    {
        "name": "arxiv_search",
        "description": (
            "Search arXiv for academic papers. Returns titles, authors, abstracts, dates, "
            "and PDF links. Covers cs.AI, cs.LG, cs.CV, physics, math, quantbio, econ, and more. "
            "Use for literature review, finding cutting-edge methods, checking novelty, citations."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Search keywords, author names, or arXiv ID (e.g. '2312.00752').",
                },
                "max_results": {
                    "type": "integer",
                    "description": "Papers to return (default 8, max 25).",
                },
                "category": {
                    "type": "string",
                    "description": "arXiv category filter (e.g. 'cs.AI', 'cs.LG', 'physics'). Optional.",
                },
                "sort_by": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": (
            "Extract the full transcript/captions from any YouTube video. "
            "Auto-generated captions work even without manual subtitles. "
            "Returns timestamped text. Use to summarize videos, extract quotes, "
            "find specific moments, or analyze content without watching."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "url": {
                    "type": "string",
                    "description": "YouTube URL or video ID (e.g. 'https://youtu.be/dQw4w9WgXcQ' or 'dQw4w9WgXcQ').",
                },
                "lang": {
                    "type": "string",
                    "description": "Preferred language code (default 'en'). Falls back to auto-generated.",
                },
                "include_timestamps": {
                    "type": "boolean",
                    "description": "Include [MM:SS] timestamps (default true).",
                },
            },
            "required": ["url"],
        },
    ...
    # ── Computer Use (full desktop) ──────────────────────────────────────────
    {
        ...
        "description": (
            "Capture a screenshot of the ENTIRE X11 virtual desktop — not just the browser tab. "
            "Sees all open windows: browsers, terminals, file managers, any GUI application. "
            "Use to verify desktop state, read UI elements, or see what applications are open. "
            "Tip: pair with desktop_control for full computer-use automation loops."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "region": {
                    "type": "object",
                    "description": "Optional crop {x, y, width, height} in pixels.",
                    "properties": {
                        "x": {"type": "integer"},
                        "y": {"type": "integer"},
                        "width": {"type": "integer"},
    ...
    },
    {
        ...
        "description": (
            "Full computer use: control the X11 virtual desktop via xdotool. "
            "Click, type, press keys, move mouse, scroll, or launch apps. "
            "Works on ANY visible GUI application — not limited to the browser. "
            "Actions: click, double_click, right_click, type, key, move, scroll, run. "
            "Always call desktop_screenshot first to confirm current desktop state."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "x": {"type": "integer", "description": "X pixel coordinate (click/move)."},
                "y": {"type": "integer", "description": "Y pixel coordinate (click/move)."},
                "text": {"type": "string", "description": "Text to type or key combo (e.g. 'ctrl+c', 'Return', 'super')."},
                "command": {"type": "string", "description": "Shell command to run (for 'run' action)."},
    ...
    # ── Persistent Jupyter Kernel ────────────────────────────────────────────
    {
        "name": "jupyter_exec",
        "description": (
            "Execute Python in a PERSISTENT kernel — variables, imports, DataFrames, and "
            "trained models all survive between calls within the same session_id. "
            "Returns stdout, text outputs, and inline matplotlib/seaborn/plotly plots as images. "
            "Full scientific stack: numpy, pandas, matplotlib, scipy, sklearn, seaborn, plotly. "
            "Install packages with !pip install. "
            "Unlike code_run (stateless subprocess), jupyter_exec remembers everything: "
            "load a CSV once, explore and visualize across many follow-up calls. "
            "Use reset=true to wipe state and start fresh."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                ...
                "session_id": {
                    "type": "string",
                    "description": "Kernel session name (default 'default'). Different IDs = isolated environments.",
                },
                "timeout": {
                    "type": "integer",
                ...
                "reset": {
                    "type": "boolean",
                    "description": "Restart the kernel before executing, clearing all state (default false).",
                },
            },
            "required": ["code"],
        },
...
# -----------------------------------------------------------------------
# plan_task — orchestration planner
# -----------------------------------------------------------------------
_plan_task_schema: dict = {
    "name": "plan_task",
    "description": (
        "Generate a step-by-step tool execution plan for any task. "
        "Call this FIRST when a task requires multiple tools or the right sequence is unclear. "
        "It queries the local LLM with the full tool catalog and returns an ordered JSON plan "
        "with exact tool names, argument values, and inter-step dependencies. "
        "The plan tells you exactly which tools to call and in what order — follow it sequentially. "
        "Supports all 90+ tools in this MCP server plus any extra tools you describe."
    ),
    "inputSchema": {
        "type": "object",
        "properties": {
            "task": {
                ...
                "description": (
                    ...
                    "Be specific: include file names, target subjects, desired output format."
                ),
            },
            "context": {
                "type": "string",
                "description": (
                    "Optional extra context: files already available, previous results, "
                    "user preferences, or constraints the planner should respect."
                ),
            },
            "extra_tools": {
                "type": "string",
                "description": (
                    "Optional description of additional tools not in this MCP server "
                    "(e.g. from other connected MCPs). Format: one tool per line as "
                    "'tool_name(param1, param2) — description'."
                ),
            },
            "max_steps": {
                "type": "integer",
                "description": "Maximum number of steps in the plan (default 10, max 20).",
            },
        },
        "required": ["task"],
    },
}
_TOOLS.append(_plan_task_schema)

# ---------------------------------------------------------------------------
# get_system_instructions — returns the planner directive for any MCP client
# ---------------------------------------------------------------------------
_TOOLS.append({
    "name": "get_system_instructions",
    "description": (
        "Returns the recommended system prompt directive for using this MCP server effectively. "
        "Call this at the start of a session (or when unsure how to proceed) to learn the "
        "orchestration workflow: when to use plan_task, how to chain tools, and best practices. "
...
Vision tools:
  face_recognize   | params: path, style[anime/photo], min_neighbors, scale_factor | detect/recognize faces; use style=anime for illustrated images
  vision_query     | params: path, question | ask a free-form question about an image
  detect_clothing  | params: path | detect clothing items and attributes in an image
  ocr_image        | params: path | extract text from an image via OCR
  image_describe   | params: path | generate a natural-language description of an image

Browser and Web tools:
  screenshot       | params: url | capture a full-page screenshot of a URL
  web_fetch        | params: url, format[text/markdown] | fetch page content as text or markdown
  web_search       | params: query, num_results | search the web via DuckDuckGo/Brave
  image_search     | params: query, num_results | search for images on the web

Code and Data tools:
  code_run         | params: code, language[python/bash] | execute code in a sandbox
  run_javascript   | params: code | execute JavaScript via Node.js
  jupyter_exec     | params: code, kernel | run code in a Jupyter kernel with state

Memory and Knowledge tools:
  memory_store     | params: key, value | persist a key-value fact to memory
  memory_get       | params: key | retrieve a stored fact by key
  vector_search    | params: query, top_k | semantic search over the vector store
  graph_query      | params: query | query the knowledge graph with natural language

Documents and Media tools:
  pdf_extract      | params: path | extract text and structure from a PDF
  docs_search      | params: query | search indexed documents
  youtube_transcript | params: url | fetch transcript of a YouTube video

LLM Utility tools:
  smart_summarize  | params: content, style[brief/bullets/detailed], max_words | summarize text
  structured_extract | params: content, schema_description | extract structured data from text
  inline_chat      | params: prompt, model | ask LM Studio a direct question
  smart_translate  | params: text, target_language | translate text to another language

Database tools:
  db_query         | params: sql | run a read-only SQL query against the aichat database
"""
...
class ModelRegistry:
    ...
    async def best_vision_model(self, client: httpx.AsyncClient) -> str:
        ...
        vision_candidates: list[tuple[int, str]] = []
        for m in await self.models(client):
            mid   = (m.get("id") or "").lower()
            mtype = (m.get("type") or "").lower()
            ...
            if is_vision:
                ...
                if any(k in mid for k in ("flash", "tiny", "mini", "nano", "small")):
                    ...
                    score = 2
                vision_candidates.append((score, m.get("id", "")))
    ...
    async def chat_models(self, client: httpx.AsyncClient) -> list[str]:
        ...
        Sorted by _vram_score: smallest/fastest models first.  Qualitative labels
        (tiny, small) are deprioritised below explicit parameter-count models because
        'devstral-small' is actually a large model despite its name.
        """
        if IMAGE_GEN_MODEL.strip():
            return [IMAGE_GEN_MODEL.strip()]
        candidates = []
        for m in await self.models(client):
            ...
            if "vl-" in mid or "-vl" in mid or mid.endswith("vl") or "flash" in mid:
                continue
            candidates.append((self._vram_score(mid), m.get("id", "")))
        return [orig for _, orig in sorted(candidates)]
    ...
    async def plan_models(self, client: httpx.AsyncClient) -> list[str]:
        ...
        Prefers models that fit fully in 24 GB VRAM and produce reliable JSON output.
        ...
        for m in await self.models(client):
            ...
            candidates.append((score, m.get("id", "")))
        return [orig for _, orig in sorted(candidates)]
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        try:
            ...
            if name == "get_system_instructions":
                return _text(
                    "=== aichat MCP — Orchestration Directive ===\n\n"
                    "You have access to 90+ tools via this MCP server (vision, browser, search,\n"
                    "memory, code execution, documents, media, LLM utilities, and more).\n\n"
                    "WORKFLOW RULE:\n"
                    ...
                    "EXAMPLES of when to use plan_task:\n"
                    "  • Detect faces in an image → search for the person → summarize\n"
                    "  • Screenshot a URL → OCR the image → extract structured data\n"
                    "  • Search the web → fetch top result → summarize findings\n"
                    "  • Load a PDF → chunk it → embed into vector store → answer questions\n\n"
                    "EXAMPLES of when to call a tool directly (no planning needed):\n"
                    "  • 'Search for X'  → web_search(query='X')\n"
                    "  • 'Summarize this text'  → smart_summarize(content='...')\n"
                    "  • 'Detect faces in this image'  → face_recognize(path='...')\n\n"
                    "KEY TOOLS:\n"
                    "  face_recognize(path, [style='anime'])  — use style='anime' for illustrated images\n"
                    "  web_search(query, [num_results])       — DuckDuckGo/Brave search\n"
                    "  screenshot(url)                        — full-page screenshot\n"
                    "  smart_summarize(content, [style])      — summarize (brief/bullets/detailed)\n"
                    "  memory_store(key, value) / memory_get(key)  — persistent key-value memory\n"
                    "  code_run(code, [language])             — execute Python or bash\n"
                    "  pdf_extract(path)                      — extract text from PDF\n"
                    "  plan_task(task, [context, max_steps])  — generate full execution plan\n\n"
