...
class TestHealthChecks:
    def test_mcp_health(self):
        with httpx.Client(timeout=10) as c:
            ...
            assert data.get("tools", 0) >= 100  # 102 tools expected
...
class TestToolDiscovery:
    ...
    def test_list_tools_anime_category(self):
        result = _mcp_call("list_tools_by_category", {"category": "anime"})
        text = _result_text(result)
        assert "anime_search" in text
        assert "anime_pipeline" in text
        assert "image_similarity_search" in text
        assert "saucenao_search" in text
...
class TestAnimePipeline:
    def test_pipeline_stores_and_embeds(self):
        """Full pipeline: Danbooru → dedupe → MinIO → CLIP → Qdrant."""
        result = _mcp_call("anime_pipeline", {
            "query": "rem_(re:zero)",
            "sources": ["danbooru"],
            "count": 2,
            "auto_embed": True,
        ...
        text = _result_text(result)
        assert "anime_pipeline complete" in text
        # Should have stored at least 1 image
        assert "Stored in MinIO: 0" not in text or "Images found: 0" in text
