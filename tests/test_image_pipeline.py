...
class TestMCPToolsAdvertised:
    ...
    def test_image_tools_in_tools_list(self):
        ...
        # These are now actions of the image/custom_tools mega-tools
        assert "image" in tools, "image mega-tool missing from tools/list"
        assert "custom_tools" in tools, "custom_tools mega-tool missing from tools/list"
        image_actions = tools["image"]["inputSchema"]["properties"]["action"]["enum"]
        for expected in ("crop", "zoom", "scan", "enhance"):
            assert expected in image_actions, f"image action '{expected}' missing"
        ct_actions = tools["custom_tools"]["inputSchema"]["properties"]["action"]["enum"]
        assert "call" in ct_actions, "custom_tools action 'call' missing"
    ...
    def test_all_image_tools_have_required_path_param(self):
        ...
        # image mega-tool has 'path' property for all path-based actions
        schema = tools_by_name["image"]["inputSchema"]
        assert "path" in schema["properties"], "image mega-tool missing 'path' property"
...
class TestDatabasePagination:
    ...
    def test_search_articles_with_offset(self):
        ...
        page0 = r0.json()["results"]
        page1 = r1.json()["results"]
        assert len(page0) == 2
        assert len(page1) == 1
...
class TestNewToolsAdvertised:
    # These are now actions within mega-tools
    _IMAGE_ACTIONS = ["stitch", "diff", "annotate"]
    _WEB_ACTIONS   = ["extract"]
    _BROWSER_ACTIONS = ["bulk_screenshot", "scroll_screenshot"]
    ...
    def test_all_new_tools_in_tools_list(self):
        ...
        for action in self._IMAGE_ACTIONS:
            assert action in tools["image"]["inputSchema"]["properties"]["action"]["enum"], \
                f"image action '{action}' missing"
        for action in self._WEB_ACTIONS:
            assert action in tools["web"]["inputSchema"]["properties"]["action"]["enum"], \
                f"web action '{action}' missing"
        for action in self._BROWSER_ACTIONS:
            assert action in tools["browser"]["inputSchema"]["properties"]["action"]["enum"], \
                f"browser action '{action}' missing"
    ...
    def test_new_tools_in_stdio_schema(self):
        from aichat.mcp_server import _TOOL_SCHEMAS
        by_name = {t["name"]: t for t in _TOOL_SCHEMAS}
        # The stdio server uses individual tool names (separate implementation from HTTP server).
        # Verify key capabilities are present in some form.
        image_related = [n for n in by_name if "image" in n or "browser" in n or "screenshot" in n]
        assert len(image_related) > 0, f"No image/browser tools found in stdio schema: {list(by_name.keys())[:10]}"
...
class TestImageGeneration:
    ...
    def test_image_generate_helpful_error_on_failure(self):
        ...
        assert len(text.strip()) > 0, f"Expected an error message, got empty response"
        # Error should indicate the failure reason (endpoint, model, or generation)
        assert any(kw in text.lower() for kw in (
            "lm studio", "image_gen_base_url", "model", "endpoint", "generat", "error"
        )), f"Error message should be informative, got: {text[:300]}"
    ...
    def test_gen_tools_advertised_in_tools_list(self):
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        # generate, edit, upscale are now actions of the image mega-tool
        assert "image" in tools, "image mega-tool missing from tools/list"
        image_actions = tools["image"]["inputSchema"]["properties"]["action"]["enum"]
        for action in ("generate", "edit", "upscale"):
            assert action in image_actions, f"image action '{action}' missing"
...
class TestBrowserV6Schemas:
    ...
    def test_screenshot_tool_has_find_image_in_mcp_http_schema(self):
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        # screenshot is now browser mega-tool action; find_image is a browser property
        assert "browser" in tools
        props = tools["browser"]["inputSchema"]["properties"]
        assert "find_image" in props, "find_image missing from HTTP MCP browser schema"
    ...
    def test_browser_tool_has_save_image_actions_in_mcp_http_schema(self):
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        enum_vals = tools["browser"]["inputSchema"]["properties"]["action"]["enum"]
        for action in ("save_images", "list_images"):
            assert action in enum_vals, f"{action} missing from HTTP browser action enum: {enum_vals}"
...
class TestBrowserImageDownload:
    ...
    def test_browser_save_images_in_mcp_http_schema(self):
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        # save_images and download_images are actions of the browser mega-tool
        assert "browser" in tools, "browser mega-tool missing from HTTP MCP tools"
        actions = tools["browser"]["inputSchema"]["properties"]["action"]["enum"]
        assert "save_images" in actions, "save_images action missing from browser"
        assert "download_images" in actions or "list_images" in actions, \
            "download_images/list_images action missing from browser"
    ...
    def test_image_search_in_mcp_http_schema(self):
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        # image_search is now image mega-tool with action=search
        assert "image" in tools, "image mega-tool missing from HTTP MCP tools"
        props = tools["image"]["inputSchema"]["properties"]
        assert "query" in props
        assert "count" in props or "limit" in props
        assert "offset" in props, "image HTTP schema missing 'offset'"
...
class TestNewLMStudioTools:
    ...
    def test_embeddings_store_endpoint(self):
        ...
        data = r.json()
        assert "stored" in data or data.get("status") in ("stored", "updated"), \
    ...
    def test_mcp_new_7_tools_in_tools_list(self):
        """These tools are now actions within mega-tools."""
        r = httpx.post(
            f"{MCP_URL}/mcp",
        ...
        tools = {t["name"]: t for t in r.json()["result"]["tools"]}
        # tts → media.tts
        assert "tts" in tools["media"]["inputSchema"]["properties"]["action"]["enum"]
        # embed_store/embed_search → vector.embed_store / vector.embed_search
        v_actions = tools["vector"]["inputSchema"]["properties"]["action"]["enum"]
        assert "embed_store" in v_actions or "store" in v_actions
        assert "embed_search" in v_actions or "search" in v_actions
        # code_run → code.python/javascript
        c_actions = tools["code"]["inputSchema"]["properties"]["action"]["enum"]
        assert "python" in c_actions or "javascript" in c_actions
        # smart_summarize → web.summarize
        assert "summarize" in tools["web"]["inputSchema"]["properties"]["action"]["enum"]
        # image_caption → image.caption
        assert "caption" in tools["image"]["inputSchema"]["properties"]["action"]["enum"]
