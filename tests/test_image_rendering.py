...
skip_load = pytest.mark.skipif(not _LOAD_OK, reason="docker/mcp/app.py load failed")
...
class TestImageRemixSchema:
    """Verify the image remix action is in the image mega-tool."""

    def _image_tool(self) -> dict:
    ...
    def test_image_remix_in_tools_list(self):
        ...
        assert tool, "image mega-tool not found in _TOOLS"
        actions = tool["inputSchema"]["properties"]["action"]["enum"]
        assert "remix" in actions, f"remix action missing from image mega-tool: {actions}"

    def test_image_remix_has_path_and_prompt_required(self):
        tool = self._image_tool()
        props = tool.get("inputSchema", {}).get("properties", {})
        # path and prompt are properties of the image mega-tool
    ...
    def test_image_remix_description_mentions_style_or_remix(self):
        ...
        assert any(kw in desc for kw in ("image", "generate", "edit", "remix", "transform", "crop")), (
            f"Description missing expected keywords: {desc[:120]}"
        )
