...
from .vision import VisionMCPTool
...
class ToolManager:
    def __init__(self, max_tool_calls_per_turn: int = 6) -> None:
        ...
        # Vision MCP service (face recognition + GPU inference on RTX 3090).
        self.vision = VisionMCPTool(_VISION_MCP_URL)
    ...
    async def run_shell(
        ...
        if _is_dangerous(command):
            raise ToolDeniedError("Blocked: potentially destructive command refused for safety.")
                ...
                try:
                    ...
                    with _PIL.open(_bio.BytesIO(raw)) as pil:
                        ...
                        if pil.width > 1280 or pil.height > 1024:
                            pil.thumbnail((1280, 1024), _PIL.Resampling.LANCZOS)
                        buf = _bio.BytesIO()
    ...
    # ------------------------------------------------------------------
    # Vision MCP tools (face recognition via CompreFace + Triton inference)
    # ------------------------------------------------------------------

    async def run_recognize_face(
        self,
        image_url: str | None,
        ...
        await self._check_approval(mode, "recognize_face", confirmer)
        return await self.vision.recognize_face(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            limit=limit, min_confidence=min_confidence,
        )

    async def run_verify_face(
        self,
        ...
        await self._check_approval(mode, "verify_face", confirmer)
        return await self.vision.verify_face(
            image_a_url=image_a_url, image_b_url=image_b_url,
            image_a_base64=image_a_base64, image_b_base64=image_b_base64,
        )

    async def run_detect_faces(
        self,
        image_url: str | None,
        image_base64: str | None,
        image_file: str | None,
        ...
        await self._check_approval(mode, "detect_faces", confirmer)
        return await self.vision.detect_faces(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            min_confidence=min_confidence, return_landmarks=return_landmarks,
        )

    async def run_enroll_face(
        self,
        subject_name: str,
        image_url: str | None,
        image_base64: str | None,
        ...
        await self._check_approval(mode, "enroll_face", confirmer)
        return await self.vision.enroll_face(
            subject_name=subject_name,
            image_url=image_url, image_base64=image_base64, image_file=image_file,
        )

    async def run_list_face_subjects(
        self,
        mode: ApprovalMode,
        confirmer: Callable[[str], Awaitable[bool]] | None,
    ) -> dict[str, object]:
        await self._check_approval(mode, "list_face_subjects", confirmer)
        return await self.vision.list_face_subjects()

    async def run_delete_face_subject(
        self,
        subject_name: str,
        mode: ApprovalMode,
        confirmer: Callable[[str], Awaitable[bool]] | None,
    ) -> dict[str, object]:
        await self._check_approval(mode, "delete_face_subject", confirmer)
        return await self.vision.delete_face_subject(subject_name)

    async def run_detect_objects(
        self,
        image_url: str | None,
        image_base64: str | None,
        image_file: str | None,
        ...
        await self._check_approval(mode, "detect_objects", confirmer)
        return await self.vision.detect_objects(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            confidence_threshold=confidence_threshold,
        )

    async def run_classify_image(
        self,
        image_url: str | None,
        image_base64: str | None,
        image_file: str | None,
        ...
        await self._check_approval(mode, "classify_image", confirmer)
        return await self.vision.classify_image(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            top_k=top_k,
        )

    async def run_detect_clothing(
        self,
        image_url: str | None,
        image_base64: str | None,
        image_file: str | None,
        ...
        await self._check_approval(mode, "detect_clothing", confirmer)
        return await self.vision.detect_clothing(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            confidence_threshold=confidence_threshold,
        )

    async def run_embed_image(
        self,
        image_url: str | None,
        image_base64: str | None,
        image_file: str | None,
        ...
        await self._check_approval(mode, "embed_image", confirmer)
        return await self.vision.embed_image(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            normalize=normalize,
        )
    ...
    async def run_analyze_image(
        ...
        await self._check_approval(mode, "analyze_image", confirmer)
        return await self.vision.analyze_image(
            image_url=image_url, image_base64=image_base64, image_file=image_file,
            include_objects=include_objects, include_labels=include_labels,
    ...
    def tool_definitions(self, shell_enabled: bool) -> list[dict[str, object]]:
        ...
        # ----------------------------------------------------------
        # Vision — face recognition (CompreFace on RTX 3090)
        # ----------------------------------------------------------
        tools.append(
            {
                "type": "function",
                "function": {
                    "name": "recognize_face",
                    "description": (
                        "Identify faces in an image by comparing against enrolled subjects in the "
                        "face recognition database (CompreFace running on the RTX 3090 GPU). "
                        "Returns matches with subject names and similarity scores. "
                        "Provide either image_url (public URL), image_base64 (base64 string), "
                        "or image_file (server path under /workspace or /tmp)."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "limit": {"type": "integer", "description": "Max matches per face (default 5)."},
                            "min_confidence": {"type": "number", "description": "Minimum similarity score 0–1 (default 0.7)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": (
                        ...
                        "Returns verified (bool) and similarity score."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "image_a_url": {"type": "string", "description": "URL of first image."},
                            "image_b_url": {"type": "string", "description": "URL of second image."},
                            "image_a_base64": {"type": "string", "description": "Base64 first image."},
                            "image_b_base64": {"type": "string", "description": "Base64 second image."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": (
                        ...
                        "Returns bounding boxes, count, and optional landmarks, age, gender, emotion."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "min_confidence": {"type": "number", "description": "Detection threshold 0–1 (default 0.8)."},
                            "return_landmarks": {"type": "boolean", "description": "Include facial landmarks (default false)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "subject_name": {"type": "string", "description": "Name to register this face under."},
                            "image_url": {"type": "string", "description": "Public URL of the image."},
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                        },
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": "List all enrolled face subjects in the recognition database.",
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": "Delete all face enrollments for a named subject.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "subject_name": {"type": "string", "description": "Subject name to delete."},
                        },
                        "required": ["subject_name"],
                    },
                },
        ...
        # ----------------------------------------------------------
        # Vision — GPU inference (Triton on RTX 3090)
        # ----------------------------------------------------------
        tools.append(
            {
                "type": "function",
                "function": {
                    ...
                    "description": (
                        ...
                        "running on the RTX 3090 GPU via Triton Inference Server. "
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "confidence_threshold": {"type": "number", "description": "Detection threshold 0–1 (default 0.25)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": (
                        ...
                        "running on the RTX 3090 GPU. Returns top-k predicted categories with confidence scores."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "top_k": {"type": "integer", "description": "Number of top predictions to return (default 5)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    ...
                    "description": (
                        "Detect clothing items and fashion categories in an image using "
                        "FashionCLIP zero-shot classification on the RTX 3090. "
                        "Identifies garment types (t-shirt, dress, jacket, shoes, etc.) with confidence scores."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "confidence_threshold": {"type": "number", "description": "Minimum confidence 0–1 (default 0.3)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    "name": "embed_image",
                    "description": (
                        "Embed an image into a 512-dimensional CLIP ViT-B/32 vector using the RTX 3090. "
                        "Useful for semantic image search, similarity comparison, and multi-modal retrieval."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "normalize": {"type": "boolean", "description": "Normalize to unit vector (default true)."},
                        },
                    },
                },
            }
        ...
        tools.append(
            {
                ...
                "function": {
                    "name": "analyze_image",
                    "description": (
                        "Full GPU scene analysis combining object detection (YOLOv8n) and image classification "
                        "(EfficientNet-B0) in a single call. Optionally include a CLIP embedding. "
                        "Best for when you need a comprehensive understanding of image content."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "image_base64": {"type": "string", "description": "Base64-encoded image."},
                            "image_file": {"type": "string", "description": "Server-local file path."},
                            "include_objects": {"type": "boolean", "description": "Run object detection (default true)."},
                            "include_labels": {"type": "boolean", "description": "Run image classification (default true)."},
                            "include_embedding": {"type": "boolean", "description": "Include CLIP embedding (default false)."},
                            "confidence_threshold": {"type": "number", "description": "Detection threshold (default 0.25)."},
                            "top_k": {"type": "integer", "description": "Top-k classification results (default 5)."},
                        },
                    },
                },
            }
