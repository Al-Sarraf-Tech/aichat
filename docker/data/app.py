    ...
    return {
        "images": [
            {"url": r[0], "host_path": r[1], "alt_text": r[2],
             "subject": r[3], "phash": r[4], "quality_score": r[5], "stored_at": str(r[6])}
            for r in rows
        ],
        "count": len(rows),
    }
...
def cache_store(payload: dict) -> dict:
    # Accept both key/value (generic) and url/content (MCP domain-specific) field names
...
def _build_nx_graph() -> nx.DiGraph:
    G = nx.DiGraph()
    with _sqlite(GRAPH_DB) as con:
        # Load all nodes first so isolated nodes are visible to has_node()
        for row in con.execute("SELECT id FROM nodes"):
            G.add_node(row["id"])
...
def graph_get_neighbors(node_id: str) -> dict:
    with _sqlite(GRAPH_DB) as con:
        row = con.execute("SELECT 1 FROM nodes WHERE id=?", (node_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail=f"Node '{node_id}' not found")
        edges = con.execute(
            "SELECT e.to_id, e.type, n.properties FROM edges e "
            "LEFT JOIN nodes n ON n.id=e.to_id WHERE e.from_id=?",
            (node_id,),
        ).fetchall()
    neighbors = [
        {"id": r[0], "edge_type": r[1], "properties": json.loads(r[2] or "{}")}
        for r in edges
    ]
...
# ---------------------------------------------------------------------------
# Embeddings router — SQLite-backed embedding store with cosine similarity
# ---------------------------------------------------------------------------

EMBED_DB = _SQLITE_ROOT / "embeddings.db"

def _init_embed_db() -> None:
    with _sqlite(EMBED_DB) as con:
        con.execute("""
            CREATE TABLE IF NOT EXISTS embeddings (
                ...
                model    TEXT DEFAULT '',
                topic    TEXT DEFAULT '',
                stored_at REAL DEFAULT (unixepoch())
            )
        """)
        con.commit()
...
def embed_store(payload: dict) -> dict:
    ...
    with _sqlite(EMBED_DB) as con:
        con.execute(
            "INSERT OR REPLACE INTO embeddings (key, content, vector, model, topic, stored_at) "
            "VALUES (?, ?, ?, ?, ?, unixepoch())",
...
def embed_search(payload: dict) -> dict:
    ...
    with _sqlite(EMBED_DB) as con:
        sql = "SELECT key, content, vector, model, topic FROM embeddings"
        rows = con.execute(sql + (" WHERE topic=?" if topic else ""),
                           (topic,) if topic else ()).fetchall()

    scored = []
    for key, content, vec_json, model, row_topic in rows:
        try:
            vec = json.loads(vec_json)
            dot = sum(a * b for a, b in zip(query_vec, vec))
            v_norm = math.sqrt(sum(v * v for v in vec)) or 1.0
            score = dot / (q_norm * v_norm)
            scored.append({"key": key, "content": content[:500], "score": round(score, 4),
                            "model": model, "topic": row_topic})
        except Exception:
            continue
